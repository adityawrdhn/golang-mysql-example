...
type User struct {
	Id           int    `json:"id"  orm:"column(id);pk;unique;int(5)"`
	...
	LastLogin    int64  `json:"last_login" orm:"column(last_login);size(20)"`
	Location     string `json:"location" orm:"column(location);size(50)"`
	Phone        string `json:"phone" orm:"column(phone);size(13)"`
	Gender       string `json:"gender" orm:"column(gender);size(1)"`                  // 0:Male, 1: Female, 2: undefined
	Status       string `json:"status" orm:"column(status);size(1)"`                  // 0: enabled, 1:disabled
	Fbprofile_id *int64 `json:"fbprofile_id" orm:"column(fbprofile_id);size(5);null"` // 0: enabled, 1:disabled
...
func CheckUsernameExceptMe(username string, token string) bool {
	exist := Users().Filter("Username", username).Filter("Token__ne", token).Exist()
	return exist
}
...
func CheckEmailExceptMe(email string, token string) bool {
	exist := Users().Filter("Email", email).Filter("Token__ne", token).Exist()
	return exist
}
...
func (m *User) UpdatePassword(newpass string, token string) (err error) {
	o := orm.NewOrm()
	user, err := GetUserByToken(token)
	if err != nil {
		return nil, err
	}
	passwordHash, err := utils.GeneratePassHash(newpass, user.Salt)
	if err != nil {
		return nil, err
	}
...
// func Activate(code string, username string) (bool, *User) {
// 	o := orm.NewOrm()
// 	user, err := GetUserByUserName(username)
// 	if err != nil {
// 		return false, nil
...
func AddUser(m *User) (*User, error) {
	...
	et := utils.EasyToken{
		Username: m.Username,
		Uid:      0,
		Expires:  time.Now().Unix() + 24*365*3600,
	}
	status := "0"
...
func UpdateUserByToken(m *User, token string) (err error) {
	o := orm.NewOrm()
	v := User{Token: token}
	user := User{
		Name:     m.Name,
	...
	if err = o.Read(&v, "Token"); err == nil {
		fmt.Println(v)
		var num int64
		if num, err = o.Update(&user, "Name", "Username", "Email", "Location", "Phone", "Gender"); err == nil {
			fmt.Println("Number of records updated in database:", num)
		}
	}
	return err
...
func GetUsername(id int) string {
	...
	if err != nil {
		...
		var user User
		o := orm.NewOrm()
		o.QueryTable(TableName("userprofile")).Filter("Id", id).One(&user, "username")
